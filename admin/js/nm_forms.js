...
function renameForm(formSource)
{    
    var inputSourceId='nm_form_id_'+formSource;
    var inputSourceTitle='nm_form_title_'+formSource;
    
    var inputTarget='new_form_name_'+formSource;
    var nm_form_title = document.getElementById(inputTarget).value;
    ...
    if (!confirm("Are you sure you want to rename '"+document.getElementById(inputSourceTitle).value+"' Form into '"+nm_form_title+"' ?"))
    return false;

  document.getElementById("renaming_targetform_name").value=nm_form_title;
  document.getElementById("renaming_targetform_id").value=nm_form_id;
  document.getElementById("renaming_sourceform_id").value=formSource;
  
  document.getElementById('nm_all_forms').action="?page=nm_forms_list";
  document.getElementById('nm_all_forms').submit();
}

function modifyForm(formSource) {
	//alert("Modifying "+formSource);
	if (formSource=='')
		{
			return false;
		}
	//alert("Missouri");
	document.getElementById('modifying_sourceform_id').value=formSource;
	document.getElementById('nm_all_forms').action="?page=nm_forms&modifying_sourceform_id="+formSource;
	document.getElementById('nm_all_forms').submit();
}

function removeForm(formSource) {
	if (!confirm("Are you sure you want to remove "+formSource+" Form ?"))
		return false;
	...
	//alert("Missouri");
	document.getElementById('removing_sourceform_id').value=formSource;
	document.getElementById('nm_all_forms').action="?page=nm_forms_list";
	document.getElementById('nm_all_forms').submit();
}

	function duplicateForm(formSource) {

		var inputSource='new_form_name_'+formSource;
		var nm_form_title = document.getElementById(inputSource).value;
		...
		document.getElementById('duplicated_targetform_name').value=nm_form_title;
		document.getElementById('duplicated_targetform_id').value=nm_form_id;
		document.getElementById('duplicated_sourceform_id').value=formSource;
...
jQuery(document).ready(function(){
	...
	jQuery(document).on( 'click', '#add_form',function(event){
		...
		//alert ("Adding a new form" + nm_form_id);
				...
				+'<div class="nm_field_settings" style="display:block;">'
								...
								+'<table class="nm_table">'
									...
									+'<tr>'
										...
										+'<td><input style="margin-bottom: 10px" id="autoreply" type="checkbox" name="nm_f['+ nm_form_id + '][autoreply]"><input id="autoreply-subject"'
										+'name="nm_f[' + nm_form_id + '][autoreply_subject]" placeholder="Autoreply Subject" type="text" style="margin-bottom: 10px"/>'
										+'<textarea id="autoreply-text"name="nm_f[' + nm_form_id + '][autoreply_msg]" placeholder="Autoreply message"></textarea></td>'
									+'</tr>'
								+'</table>'
							+'</div>'
			+'</li>'
	...
	jQuery(document).on( 'change', '.nm_field_type',function(event){
		...
		}else if(option == 'html' || option == 'startingzone') {
...
function addAutoReply() {
	jQuery('#autoreply').on('change', function() {
		if(jQuery(this).is(":checked")) {
			jQuery('#autoreply-text').css('display', 'block');
			jQuery('#autoreply-subject').css('display', 'block');
		}else {
			jQuery('#autoreply-text').css('display', 'none');
			jQuery('#autoreply-subject').css('display', 'none');
		}
	});
}
